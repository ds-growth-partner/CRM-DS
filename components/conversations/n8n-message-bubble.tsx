...
export function N8nMessageBubble({ entry }: N8nMessageBubbleProps) {
  const { type, content, isToolCall, toolName } = parseMessage(entry.message)
  ...
  return (
    <div className={cn('flex mb-2 px-4', isOutbound ? 'justify-end' : 'justify-start')}>
      ...
      {isOutbound && (
        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 border border-emerald-500/20 ml-1.5 mt-1">
